...
final class DbActivityRepository extends AbstractDbRepository implements ActivityRepositoryInterface
{
    ...
    {
        $deleted = $this->wpdb->delete($this->table('mh_activity'), ['id' => $id], ['%d']);
    ...
    {
        $sql = $this->wpdb->prepare('SELECT * FROM ' . $this->table('mh_activity') . ' WHERE id = %d', $id);
    ...
    public function list(DateRange $range, array $filters = []): array
    {
        ...
        $sql = 'SELECT * FROM ' . $this->table('mh_activity') . ' WHERE ' . implode(' AND ', $conditions) . ' ORDER BY occurred_at DESC LIMIT %d';
